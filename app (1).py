...
# ══════════════════════════════════════════════
# Supabase — HTTP مباشر بدون مكتبات
# ══════════════════════════════════════════════
...
def db_get(table, params=""):
    r = requests.get(f"{SUPABASE_URL}/rest/v1/{table}?{params}&order=id", headers=HEADERS)
...
def db_update(table, wid, data):
    data["updated_at"] = datetime.now().isoformat()
    r = requests.patch(f"{SUPABASE_URL}/rest/v1/{table}?id=eq.{wid}", json=data, headers=HEADERS)
    return r.ok

def db_delete(table, wid):
    r = requests.delete(f"{SUPABASE_URL}/rest/v1/{table}?id=eq.{wid}", headers=HEADERS)
    return r.ok

def db_get_workers():
    return db_get("workers", "select=*")

def db_save_worker(w):
    wid = w.get("id")
    w.pop("id", None)
    w.pop("created_at", None)
    w.pop("updated_at", None)
    if wid:
        db_update("workers", wid, w)
    else:
        db_insert("workers", w)

def db_delete_worker(wid):
    return db_delete("workers", wid)

def db_get_note():
    res = db_get("settings", "key=eq.company_note&select=value")
    return res[0]["value"] if res else ""

def db_save_note(note):
    requests.patch(f"{SUPABASE_URL}/rest/v1/settings?key=eq.company_note",
        json={"value": note}, headers=HEADERS)
...
.kpi-card .lbl { font-size: 13px; color: #555; margin-top: 6px; font-weight: 700; }

.styled-table {
    width: 100%; border-collapse: collapse; background: white;
    border-radius: 12px; overflow: hidden;
    box-shadow: 0 4px 15px rgba(0,0,0,0.08); font-size: 14px;
}
.styled-table th {
...
.styled-table tr:hover { background: #E3EDFF; }

.alert-r { background:#FFF0EE; border-right:5px solid #C00000; padding:12px 16px; border-radius:10px; margin:5px 0; }
.alert-o { background:#FFFBEE; border-right:5px solid #BF8F00; padding:12px 16px; border-radius:10px; margin:5px 0; }
.alert-g { background:#F0FFF4; border-right:5px solid #1E7145; padding:12px 16px; border-radius:10px; margin:5px 0; }

section[data-testid="stSidebar"] { background: linear-gradient(180deg, #1F3864 0%, #2E5496 100%) !important; }
...
def sel(opts, val): return opts.index(val) if val in opts else 0
...
def worker_form(w, kp=""):
    ...
    c1,c2 = st.columns(2)
    w["name"]  = c1.text_input("الاسم الكامل *", value=w.get("name",""), key=f"{kp}_n")
    w["iqama"] = c2.text_input("رقم الإقامة",    value=w.get("iqama",""), key=f"{kp}_iq")
    w["email"] = c1.text_input("📧 الإيميل",     value=w.get("email",""), key=f"{kp}_em")
    ...
    with tabs[0]:
        has = st.radio("هل يملك رخصة مسبقاً؟",
            ["لا يملك","نعم — يملك رخصة مسبقاً"],
            index=1 if w.get("has_license_already")=="نعم" else 0,
            key=f"{kp}_hl", horizontal=True)
        w["has_license_already"] = "نعم" if "نعم" in has else ""
        if w["has_license_already"] == "نعم":
            st.success("✅ يملك رخصة مسبقاً")
            d = st.date_input("تاريخ الرخصة المسبقة", value=parse_date(w.get("existing_license_date")), key=f"{kp}_eld")
            w["existing_license_date"] = fmt_date(d)
        else:
            st.info("سيمر بكل مراحل التقييم والتدريب والاختبار")

    with tabs[1]:
        c1,c2,c3,c4 = st.columns(4)
        d = c1.date_input("تاريخ التقييم", value=parse_date(w.get("eval_date")), key=f"{kp}_ed")
        w["eval_date"]   = fmt_date(d)
        w["eval_time"]   = c2.text_input("الوقت", value=w.get("eval_time",""), placeholder="10:00", key=f"{kp}_et")
        w["eval_hours"]  = c3.selectbox("الساعات", HOURS, index=sel(HOURS,w.get("eval_hours","")), key=f"{kp}_eh")
        w["eval_result"] = c4.selectbox("النتيجة", RES,   index=sel(RES,  w.get("eval_result","")), key=f"{kp}_er")

    with tabs[2]:
        c1,c2 = st.columns(2)
        with c1:
            st.markdown("**موعد 1**")
            d = st.date_input("التاريخ", value=parse_date(w.get("train_n1_date")), key=f"{kp}_tn1d")
            w["train_n1_date"] = fmt_date(d)
            w["train_n1_time"] = st.text_input("الوقت", value=w.get("train_n1_time",""), placeholder="09:00", key=f"{kp}_tn1t")
        with c2:
            st.markdown("**موعد 2**")
            d = st.date_input("التاريخ", value=parse_date(w.get("train_n2_date")), key=f"{kp}_tn2d")
            w["train_n2_date"] = fmt_date(d)
            w["train_n2_time"] = st.text_input("الوقت", value=w.get("train_n2_time",""), placeholder="09:00", key=f"{kp}_tn2t")
        w["train_n_attend"] = st.selectbox("الحضور", ATTEND, index=sel(ATTEND,w.get("train_n_attend","")), key=f"{kp}_tna")

    with tabs[3]:
        for i in range(1,4):
            with st.expander(f"محاولة {i}", expanded=(i==1)):
                c1,c2,c3 = st.columns(3)
                d = c1.date_input("التاريخ", value=parse_date(w.get(f"exam_n{i}_date")), key=f"{kp}_en{i}d")
                w[f"exam_n{i}_date"]   = fmt_date(d)
                w[f"exam_n{i}_time"]   = c2.text_input("الوقت", value=w.get(f"exam_n{i}_time",""), key=f"{kp}_en{i}t")
                w[f"exam_n{i}_result"] = c3.selectbox("النتيجة", RES, index=sel(RES,w.get(f"exam_n{i}_result","")), key=f"{kp}_en{i}r")

    with tabs[4]:
        c1,c2,c3 = st.columns(3)
        d = c1.date_input("التاريخ", value=parse_date(w.get("train_e_date")), key=f"{kp}_ted")
        w["train_e_date"]   = fmt_date(d)
        w["train_e_time"]   = c2.text_input("الوقت", value=w.get("train_e_time",""), placeholder="14:00", key=f"{kp}_tet")
        w["train_e_attend"] = c3.selectbox("الحضور", ATTEND, index=sel(ATTEND,w.get("train_e_attend","")), key=f"{kp}_tea")

    with tabs[5]:
        for i in range(1,4):
            with st.expander(f"محاولة {i}", expanded=(i==1)):
                c1,c2,c3 = st.columns(3)
                d = c1.date_input("التاريخ", value=parse_date(w.get(f"exam_e{i}_date")), key=f"{kp}_ee{i}d")
                w[f"exam_e{i}_date"]   = fmt_date(d)
                w[f"exam_e{i}_time"]   = c2.text_input("الوقت", value=w.get(f"exam_e{i}_time",""), key=f"{kp}_ee{i}t")
                w[f"exam_e{i}_result"] = c3.selectbox("النتيجة", RES, index=sel(RES,w.get(f"exam_e{i}_result","")), key=f"{kp}_ee{i}r")

    with tabs[6]:
        d = st.date_input("تاريخ إصدار الرخصة", value=parse_date(w.get("license_date")), key=f"{kp}_ld")
        w["license_date"] = fmt_date(d)
        st.info(f"الحالة: **{get_status(w)}**")

    with tabs[7]:
        w["notes"] = st.text_area("ملاحظات", value=w.get("notes",""), height=120, key=f"{kp}_notes")
...
def login_page():
    ...
    with c2:
        ...
        <div style="text-align:center;padding:30px;background:white;border-radius:20px;
            ...
            <h2 style="color:#1F3864;font-weight:800;margin:10px 0 5px;">نظام متابعة مراحل الرخصة</h2>
            <p style="color:#888;margin:0;">قتاد اللوجستية</p>
...
def dashboard_page(workers, note):
    ...
    with col_r:
        ...
        for stage,count in sd.items():
            ...
            st.markdown(f'<div style="margin:6px 0;display:flex;align-items:center;gap:8px;"><span style="min-width:170px;font-size:13px;font-weight:600;">{stage}</span><span style="color:{color};font-family:monospace;font-size:10px;">{bar}</span><span style="font-weight:800;color:{color};font-size:15px;">{count}</span></div>', unsafe_allow_html=True)
...
def tracking_page(workers, role):
    ...
    filtered = [w for w in workers if
        (not search or search.lower() in w.get("name","").lower() or search in w.get("iqama","")) and
    ...
    for w in filtered:
        ...
        rows  += f"""<tr>
            <td style="font-weight:800;color:#1F3864;font-size:14px;">{w.get('id','')}</td>
            ...
            <td><span style="background:{color}18;color:{color};padding:5px 14px;border-radius:20px;font-size:13px;font-weight:700;">{status}</span></td>
            <td>{fn_s}</td><td>{fe_s}</td>
            <td style="color:{sc};font-weight:700;">{stag_s}</td>
        </tr>"""
    ...
    # تعديل
    if can_edit and workers:
        st.markdown("---")
        st.markdown("### ✏️ تعديل بيانات عامل")
        names = [f"{w.get('id','')}. {w.get('name','')}" for w in workers]
        sel_w = st.selectbox("اختر العامل", names)
        wid   = int(sel_w.split(".")[0])
        w     = next((x for x in workers if x.get("id")==wid), None)
        if w:
            with st.expander(f"📝 {w.get('name','')}", expanded=True):
                w_edit = dict(w)
                w_edit = worker_form(w_edit, kp=f"e{wid}")
                cb1,cb2,_ = st.columns([1,1,3])
                if cb1.button("💾 حفظ", type="primary"):
                    db_save_worker(w_edit)
                    st.success("✅ تم الحفظ!")
                    st.rerun()
                if can_del and cb2.button("🗑️ حذف"):
                    if st.session_state.get("del_confirm") == wid:
                        ...
                        st.warning("⚠️ اضغط مجدداً للتأكيد")
...
def schedule_page(workers):
    ...
    for a in appts:
        ...
        rows += f"""<tr>
            <td style="text-align:right;font-weight:700;font-size:15px;">{a['name']}</td>
            <td><span style="background:{tc}22;color:{tc};padding:5px 12px;border-radius:20px;font-size:13px;font-weight:700;">{a['type']}</span></td>
            <td style="font-weight:600;">{a['date'].strftime('%Y/%m/%d')}</td>
            <td style="color:#7030A0;font-weight:700;font-size:15px;">{a['time']}</td>
            <td style="color:#555;">{day} ({diff_s})</td>
            <td><span style="background:{bg};color:{fg};padding:5px 12px;border-radius:20px;font-size:13px;font-weight:700;">{a['status']}</span></td>
        </tr>"""
    ...
    with col2:
        ...
        if failed:
            for w in failed:
                parts = []
                fn,fe = get_fail_n(w),get_fail_e(w)
                if fn: parts.append(f"نظري: {fn}")
                if fe: parts.append(f"عملي: {fe}")
...
def search_page(workers):
    ...
    results = [w for w in workers if
        (s_name and s_name.lower() in w.get("name","").lower()) or
    ...
    for w in results:
        ...
        with st.expander(f"👤 {w.get('name','')} — {w.get('iqama','')}", expanded=True):
            c1,c2,c3,c4 = st.columns(4)
            c1.markdown(f'<span style="background:{color}22;color:{color};padding:5px 14px;border-radius:20px;font-weight:700;">{status}</span>', unsafe_allow_html=True)
            c2.metric("رسوب نظري", get_fail_n(w))
            c3.metric("رسوب عملي", get_fail_e(w))
            stag = get_stagnant(w)
...
def main():
    ...
    with st.sidebar:
        ...
        <div style="text-align:center;padding:20px 0 10px;">
            ...
            <h3 style="color:white;margin:8px 0 4px;font-size:18px;">قتاد اللوجستية</h3>
            <div style="background:rgba(255,255,255,0.15);padding:6px 14px;border-radius:20px;
                 display:inline-block;margin:4px 0;">
                <span style="font-size:13px;font-weight:600;">{ROLES[role]['label']}: {user['name']}</span>
            </div>
            <br><small style="color:rgba(255,255,255,0.5);font-size:11px;">
        ...
        st.markdown("---")
        page = st.radio("", [
            "🏠 الرئيسية","📋 المتابعة","📅 المواعيد","📊 المؤشرات","🔍 البحث"
